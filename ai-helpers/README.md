...
Designed to delegate the complete cycle to an Orchestrator using the **[Zoo Code](https://zed.dev/extensions/zoo-code) IDE extension**, which provides native multi-agent coordination (Context Provider, Architect, Code, Debug). Sub-agents have no memory between invocations; `orchestator-memory.md` acts as the shared working memory for the duration of the plan.
...
| `orchestator-memory.md` | **[Orchestrated Flow]** Working memory: PR status, anti-redundancy inventory, critical conventions, and alert notes. Ephemeral per plan. |
...
Sub-agents invoked by the Orchestrator start as "blank slates" with no project memory. The Orchestrator feeds them only the context strictly necessary for their task. In the Orchestrated Flow, that context lives in `orchestator-memory.md`; in the Manual Flow, the user manages it directly.
